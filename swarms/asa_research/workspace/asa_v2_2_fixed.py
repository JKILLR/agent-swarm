...
# POLYSEMY NOTE: Some verbs have multiple senses across classes. We assign to the
# most syntactically frequent or etymologically primary class. Decisions:
#   write → communication (message transfer, not artifact creation)
#   cook  → change (heat transformation, not artifact creation)
#   return → motion_directed (come back, not transfer possession)
#   feel/smell/taste/look → perception (active sensing, not stative linking)
#   need  → stative (syntactic behavior matches statives)
#   hold  → contact (physical grip is etymologically primary)
# See VerbNet for full frame coverage of polysemous verbs.
VERB_TO_CLASS: Dict[str, str] = {
    ...
    'draw': 'creation', 'paint': 'creation',  # write → communication
    'sculpt': 'creation', 'carve': 'creation', 'weave': 'creation',
    'knit': 'creation', 'sew': 'creation',  # cook → change
    ...
    'lend': 'transfer', 'borrow': 'transfer',  # return → motion_directed
    ...
    'keep': 'possession', 'retain': 'possession',  # hold → contact
    ...
    'desire': 'emotion', 'crave': 'emotion',  # need → stative
    ...
    # NOTE: look/feel/smell/taste → perception (active sensing)
...
Î± controls how much linguistic compatibility affects attention:
    Score = QÂ·K^T/âˆšd + Î±Â·Compatibility

Recommended starting points:
- Î± = 0.5: Weak linguistic bias (let model learn more)
- Î± = 1.0: Balanced (default)
- Î± = 2.0: Strong linguistic bias (trust linguistics more)

Tuning strategy:
1. Train with Î± âˆˆ {0.5, 1.0, 2.0} on validation set
2. Pick Î± that minimizes perplexity
3. For downstream tasks, may need task-specific Î±

Expected behavior:
- Higher Î± â†’ More sparse attention â†’ Faster inference
- Lower Î± â†’ Model can override linguistic constraints
- Î± = 0 with mode='none' â†’ Standard transformer

Note: Î± could be made learnable, but this defeats the
...
def print_summary():
    ...
    print(f"")
    print(f"Default Î±=1.0 (see ALPHA_GUIDANCE for tuning)")
